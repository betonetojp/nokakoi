...
import { $, buildReactionEmojiTags, buildStoredReactionValue, getReactionContent, isReactionShortcodeOnly, resolveReactionCustomEmoji } from './utils.js';
...
export function showReactionModal(currentSymbol, onConfirm, settingsManager, options = {}) {
  ...
  try {
    if (typeof window !== 'undefined' && typeof window.bringModalToFront === 'function') {
      try { window.bringModalToFront(modal); } catch (e) { /* ignore */ }
    } else {
...
export function showOmochatSettingsModal(settingsManager) {
  ...
  // 保存ボタン
  saveBtn.onclick = () => {
    const val = input.value.trim() || 'xn'; // default to xn if empty
