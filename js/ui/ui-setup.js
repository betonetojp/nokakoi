...
function collapseSettingsPanels() {
  ...
  // DOM 反映後に open 状態を更新
  try { setTimeout(() => { try { /* 折りたたんだ後、何も開いていなければ投稿欄を復元する */ try { restoreComposer(); } catch (ee) { } } catch (e) { } try { restoreComposer(); } catch (ee) { } }, 50); } catch (e) { try { restoreComposer(); } catch (ee) { } }
}
...
function renderTabSettingsUI(settingsManager, container) {
  ...
  currentTabs.forEach((tab, index) => {
    ...
    chk.onchange = () => {
      ...
      saveTabSettings(settingsManager, currentTabs);
      renderTabSettingsUI(settingsManager, container); // 無効化状態を更新するために再描画
      setupTabs(settingsManager, true);
      try { window.dispatchEvent(new CustomEvent('tabsRebuilt')); } catch(e) {}
    };
  ...
  if (clearCacheBtn) {
    clearCacheBtn.onclick = async function () {
      try {
        ...
        try {
          const toastDur = 3000; // キャッシュクリア表示用の3秒間
      ...
      langSelect.onchange = function () {
        try {
          ...
          import('../utils/i18n.js').then(m => {
            ...
            try {
              const toastDur = 3000; // リロード前に短いトーストを表示する
