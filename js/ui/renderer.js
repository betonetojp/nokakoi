import { escapeHtml, fmtTime, truncateName, truncateByGraphemeVisible, processHiddenTagChars, buildReactionEmojiTags, getReactionContent, getReactionEmojiTags, replaceBadgeEmoji } from '../utils/utils.js';
import { findEventById } from '../core/state.js';
import { displayName, displayNameWithUsername, loadUserStatus } from '../features/profile/profile.js';
...
export function renderReplyContext(state, ev, nip19, settings) {
  ...
  const isOpaqueAuthor = (function (a, pk) {
    try {
      if (!a) return true; // 表示名がない場合は opaque 扱い
