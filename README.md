...
- ローカルスナップショット（プロフィール kind:0、フォロー kind:3、ミュート kind:10000、リレー kind:10002、Public chats kind:10005 のバックアップ・復元）
- タイムライン（ホーム / リレー / 自分 / 通知。タブの表示・順序変更）
- NIP-22 (kind:1111) コメント（送信・受信・表示設定）
- NIP-28 チャンネル（kind:40/41 メタデータ、kind:42 の閲覧・投稿・返信、kind:10005 Public chats 参加同期、共通 composer 統合）
