...
- マルチアカウント管理（複数アカウントの保持・ワンタップ切り替え・新規アカウント生成・アカウント削除時の全データ完全消去）
- NIP-65 (kind:10002) リレーリスト管理（初回自動同期・上書き発行・リレー設定の自動最適化）
- プロフィール (kind:0) & リレーリスト (kind:10002) ローカルスナップショット（バックアップ・復元・管理）
- 多彩なログイン方式
  - NIP-07（ブラウザ拡張機能・連携アプリ）認証
  - `nsec` + パスキー（WebAuthn PRF生体認証）
  - `nsec` + パスワード暗号化（PBKDF2 + AES-256-GCM）
  - NIP-46（Amber連携・リモート署名／通信鍵自動分離）
- NIP-22 (kind:1111) コメント機能（送信・受信・表示設定）
- 高度なミュート機能（非公開／公開ミュートトグル、kind:0等適用範囲拡張、ミュート投稿の再折りたたみ、アカウント別完全同期）
- アバター・ヘッダーUI刷新（アクティブアカウント表示、アカウント名省略最適化）
- タイムライン＆メディア拡張（画像解像度・ファイルサイズ表示、参照先イベント動的カード化）
- カスタム絵文字（NIP-30）
- PWA
- i18n（日本語/英語）
