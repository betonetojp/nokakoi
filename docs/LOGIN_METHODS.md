...
2. 接続成功後に自動再接続の保存方法を選ぶ。パスキー保護を選ぶと、Client Key と Connect secret を WebAuthn PRF + AES-GCM で暗号化する。未保護を選ぶ場合だけ次の localStorage キーに Client Key を保存する（`appSettings` には混ぜない）
...
- パスキー保護を選んだ場合、リロード・アカウント切替での自動再接続時に生体認証または PIN が必要です
- 未保護の自動再接続は利便性を優先するため、通信鍵を読み取れる XSS や端末アクセスに対するセッションなりすましリスクが残ります
...
| 保存時の暗号 | なし | PRF → AES-256-GCM（`prf1:`） | 任意で PRF → AES-256-GCM（`nip46prf1:`）、未保護時は通信鍵を平文保存 | PBKDF2 + AES-256-GCM |
