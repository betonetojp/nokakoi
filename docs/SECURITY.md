...
| LocalStorageの第三者閲覧（NIP-46） | ⚠️ 残存リスク | ローカル通信鍵は専用キーで永続化（nsec ではない／後述） |
...
2. 設定保存時および起動時に、過去バージョンで `appSettings` に残った平文鍵を除去する
3. ログアウト時に専用キーも消去する（明示ログアウト後は再ペアが必要）
4. ユーザーの nsec は Web 側に置かない（リモートサイナー側に留まる）
